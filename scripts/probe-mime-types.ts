 ...
 * what actually works by attempting real API calls with sample files. Binary
 * sample content comes from committed fixtures in scripts/fixtures/ (real
 * media files generated by scripts/generate-fixtures.ts); types without a
 * fixture fall back to synthesized minimal content.
 *
 * Usage:
...
interface ProbeResult {
  ...
  sampleSource: 'fixture' | 'synthetic';
  inline: TestOutcome;
  fileApi: TestOutcome;
}
...
function classifyError(message: string): ErrorClass {
  ...
  if (
    ...
    /invalid (image|audio|video|media|file|document|content|data)/.test(lower) ||
    /(image|audio|video|media|file|document) (is|was) (invalid|corrupt|malformed)/.test(lower) ||
...
/**
 * Committed real-sample fixtures, generated by scripts/generate-fixtures.ts.
 * Fixture files are named by catalog extension (e.g. "png" for image/png).
 */
const FIXTURES_DIR = path.join(__dirname, 'fixtures');

/**
 * Load a real sample file for a binary MIME type. Returns null when no
 * fixture exists; the caller falls back to synthesized minimal content.
 */
function loadFixture(entry: MimeTypeEntry): Buffer | null {
  if (!entry.binary) return null;
  try {
    return fs.readFileSync(path.join(FIXTURES_DIR, entry.extension.slice(1)));
  } catch {
    return null;
  }
}
  ...
  async probe(options: {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const sampleContent = fixture ?? createSampleContent(entry);
        const sampleSource: ProbeResult['sampleSource'] = fixture ? 'fixture' : 'synthetic';
